...
for subcuenca in range(1,41):
    ...
    ##############  Para cargar la tmax mensual que hay que corregir 
    ...
    df_tmax_uncorrected = df_X.transpose()
    
    
    #####################################################
    ###############  COrreccion de las tmax###############
    
    # Reindexar los dataframes para asegurar que tienen los mismos índices y columnas
    coeficientes_correctores = coeficientes_correctores.set_index(df_tmax_uncorrected.index)
    coeficientes_correctores.columns = df_tmax_uncorrected.columns
    
    # Multiplica los dataframes elemento a elemento
    tmax_corregida = df_tmax_uncorrected.mul(coeficientes_correctores)
    
    # Formatea el nombre del archivo
    nombre_archivo = f"Corrected_tmax_monthly_mean_ECMWF_subcuenca_{subcuenca}_mes_{mes}.csv"
    
    # Define la ruta completa del archivo
    ruta = r"D:\Seasonal_forecast\Bias_corrected_forecast_monthly\tmax_mensuales_corregidas"
    ruta_completa = f"{ruta}\\{nombre_archivo}"
    
    # Guarda el resultado en un archivo CSV
    tmax_corregida.to_csv(ruta_completa, index=True)
    ...
    # Construye el nombre del archivo basado en los parámetros
    filename_pattern = f"Tmax_monthly_AEMET_subcuenca_{subcuenca}_mes_{mes}_"
    ...
    # Construye el nombre del archivo basado en los parámetros
    filename_pattern = f"raw_biasAdd_MEAN_tmax_monthly_subcuenca_{subcuenca}_mes_{mes}_"
    ...
    biasAdd = tmax_corregida - tmax_obs
    ...
    nombre_archivo = f"corrected_biasAdd_MEAN_tmax_monthly_subcuenca_{subcuenca}_mes_{mes}.csv"
