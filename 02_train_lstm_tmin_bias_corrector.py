...
for subcuenca in range(1,41):
    ...
    filename_pattern = f"coef_mean_tmin_monthly_subcuenca_{subcuenca}_mes_{mes}_"
    ...
    for _ in range(100):  # Intentos aleatorios, ajustar según necesidad
        ...
        if R2[0] > 0.65  and all(not np.isnan(r2) for r2 in R2):
            best_model = model
            best_R2 = R2
    ...
    if best_model:
        ...
        filename = f'D:\\Seasonal_forecast\\Bias_correction_LSTM\\Resultados\\LSTM_correction_tmin_monthly_subcuenca_{subcuenca}_mes_{mes}.jpeg'
        plot_model(best_model, to_file=filename, show_shapes=True, show_layer_names=False, dpi=96)
    
        filename = f'D:\\Seasonal_forecast\\Bias_correction_LSTM\\Resultados\\performance_correction_tmin_monthly_subcuenca_{subcuenca}_mes_{mes}.csv'
        results_df.to_csv(filename, index=False)
    
        predictions_filename = f'D:\\Seasonal_forecast\\Bias_correction_LSTM\\Resultados\\coef_correction_tmin_monthly_subcuenca_{subcuenca}_mes_{mes}.csv'
