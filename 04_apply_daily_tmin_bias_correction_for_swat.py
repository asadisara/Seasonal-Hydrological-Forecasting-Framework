...
for subcuenca in range(1, 41):
    ...
    for year in range(1981, 2020):
        ...
        filename_pattern = f"tmin_daily_MEAN_subcuenca_{subcuenca}_mes_{mes}_year_{year}"
        ...
        if matching_file:
            ...
            df_t_uncorrected['Tmin_Corregida'] = df_t_uncorrected['Tmin_ECMWF_mean'] * df_t_uncorrected['Coeficiente_Corrector']
            ...
            end_date = pd.Timestamp(start_date) + pd.Timedelta(days=len(tmp_min_AEMET_data)-1)
            ...
            tmp_min_prev_year = tmp_min_AEMET_df.loc[fechas_previas]
            # Convertir los datos a una serie de valores
            tmp_min_prev_year_series = tmp_min_prev_year['Temperatura_Minima'].values
    
            # Crear el DataFrame
            tmp_min_prev = pd.DataFrame({'Unnamed: 0': fechas_previas, 'Tmin_Corregida': tmp_min_prev_year_series})
            # Cambiar el formato de la columna de fecha
            tmp_min_prev['Unnamed: 0'] = tmp_min_prev['Unnamed: 0'].dt.strftime('%Y-%m-%d')
    
    
    
            # Concatenar el DataFrame replicado con df_pcp_uncorrected_extended
            # Obtener las columnas necesarias de df_t_uncorrected
            df_t_uncorrected_extended = df_t_uncorrected[['Unnamed: 0', 'Tmin_Corregida']].copy()
    
            df_t_uncorrected_extended = pd.concat([tmp_min_prev, df_t_uncorrected_extended], ignore_index=True)
           
            output_folder = "D:/Seasonal_forecast/Bias_corrected_forecast_monthly/tmp_diarias_corregidas_para_SWAT/tmin_diarias_corregidas_para_SWAT"
    
            # Nombre del archivo
            output_filename = f"Corrected_tmin_subcuenca_{subcuenca}_mes_{mes}_year_{year}.tmp"
